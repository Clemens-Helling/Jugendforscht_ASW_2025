...
class AlarmApp(tk.Tk):
    def __init__(self):
        ...
        for F in (StartPage, MenuPage, PageTwo, RFIDPage, UserPage, El_Page):
...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = ttk.Label(content_frame, text="Notifyer", font=("Arial", 24))
        ...
        self.last_name_insert.place(
            relx=0.5, rely=0.3, anchor="center"
        )  # Abstand nach oben
        self.last_name_insert.bind(
            "<FocusIn>", lambda event: self.clear_placeholder(event, "Nachname")
        )
        self.last_name_insert.bind(
            "<FocusOut>", lambda event: self.add_placeholder(event, "Nachname")
        )
        ...
        self.first_name_insert.place(
            relx=0.5, rely=0.3, anchor="center"
        )  # Abstand nach oben
        self.first_name_insert.bind(
            "<FocusIn>", lambda event: self.clear_placeholder(event, "Vorname")
        )
        self.first_name_insert.bind(
            "<FocusOut>", lambda event: self.add_placeholder(event, "Vorname")
        )
        self.first_name_insert.place(relx=0.5, rely=0.3, anchor="center")
        self.first_name_insert.bind(
            "<Return>", lambda event: self.last_name_insert.focus_set()
        )

        unclear_situation_checkbox = ttk.Checkbutton(
            content_frame,
            text="Unklare Lage",
            variable=self.unclear_situation_checkbox_state,
        )
    ...
    def toggle_textfield(self):
        ...
        if self.other_checkbox_state.get():
            self.other_entry.place(
                relx=0.5, rely=0.55, anchor="center"
            )  # Zeigt das Textfeld an
        else:
...
class MenuPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        button3 = tk.Button(
            content_frame,
            text="RFID",
            command=lambda: controller.show_frame("RFIDPage"),
        )
        ...
        button5 = tk.Button(
            content_frame,
            text="EL Daten",
            command=lambda: controller.show_frame("El_Page"),
        )
...
class PageTwo(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.tree = ttk.Treeview(
            self,
            columns=("id", "name", "lastname", "symptom", "medic", "timestamp"),
            show="headings",
        )
        ...
        self.tree.column("symptom", width=100)
...
class RFIDPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.label = tk.Label(
            content_frame, text="Zugangskontrolle", font=("Arial", 24)
        )
        ...
        self.treeview = ttk.Treeview(
            content_frame,
            columns=("Name", "Nachname", "Letzter Zugriff", "UUID"),
            show="headings",
        )
        ...
        self.delete_accsess_button = tk.Button(
            content_frame,
            text="Zugriff löschen",
            font=("Arial", 15),
            bg="red",
            fg="white",
        )
        ...
        self.add_accsess_key_button = tk.Button(
            content_frame,
            text="RFID Chip hinzufügen",
            font=("Arial", 15),
            bg="green",
            fg="white",
        )
...
class UserPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.label = tk.Label(
            content_frame, text="Benutzter hinzufügen", font=("Arial", 24)
        )
        self.label.place(relx=0.5, rely=0.1, anchor="center")
        ...
        self.privilage_combobox = ttk.Combobox(
            content_frame,
            text="Berechtigung",
            values=["Admin", "Sanitäter", "Benutzer", "Sanitäter+"],
        )
...
class El_Page(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.picked_up_checkbox = ttk.Checkbutton(
            content_frame, text="Abgeholt", variable=self.picked_up_checkbox_state
        )
        self.picked_up_checkbox.place(relx=0.2, rely=0.2, anchor="center")

        self.alone_at_home_checkbox_state = tk.BooleanVar()
        self.alone_at_home_checkbox = ttk.Checkbutton(
            content_frame,
            text="Alleine nach Hause",
            variable=self.alone_at_home_checkbox_state,
        )
        self.alone_at_home_checkbox.place(relx=0.2, rely=0.3, anchor="center")

        self.taxi_checkbox_state = tk.BooleanVar()
        self.taxi_checkbox = ttk.Checkbutton(
            content_frame,
            text="Taxi zum Arzt ins KH",
            variable=self.taxi_checkbox_state,
        )
        self.taxi_checkbox.place(relx=0.2, rely=0.4, anchor="center")

        self.ambulance_checkbox_state = tk.BooleanVar()
        self.ambulance_checkbox = ttk.Checkbutton(
            content_frame, text="Rettungsdienst", variable=self.ambulance_checkbox_state
        )
        self.ambulance_checkbox.place(relx=0.2, rely=0.5, anchor="center")

        self.hospital_checkbox_state = tk.BooleanVar()
        self.hospital_checkbox = ttk.Checkbutton(
            content_frame, text="Ins Krankenhaus", variable=self.hospital_checkbox_state
        )
        self.hospital_checkbox.place(relx=0.2, rely=0.6, anchor="center")

        self.notified_at_label = ttk.Label(
            content_frame, text="Eltern benachrichtigt um:"
        )
    ...
    def submit(self):
        ...
        if self.picked_up_checkbox_state.get():
            ...
            messagebox.showerror("Fehler", "Bitte wählen Sie eine Abholmaßnahme")
            return
        add_el_data(
            self.alert_id_entry.get(),
            abhol_massnahme,
            self.notified_at_entry.get(),
            self.notified_by_entry.get(),
            self.hospital_entry.get(),
        )
